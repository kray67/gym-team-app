...
- **Plan Focus options** (in `plan_editor_screen.dart` `_focusOptions`): Strength / Bodybuilding / Fitness / Cardio / Athletics / Olympic Weightlifting / Female Focused
- **Colored left strip pattern**: `Card > ClipRRect(borderRadius:16) > Stack > [Padding(left:4) + Positioned(left:0,top:0,bottom:0, Container(width:4, color:stripColor))]` — used on exercise cards (active workout + plan session builder) and plan list cards; strip color = superset palette color, `colorScheme.primary`, or green for active plans
- **AppBar title color**: set globally via `AppBarTheme.titleTextStyle` to `colorScheme.primary` at 22px w800 — do not override per-screen unless there is a specific reason
...
- **Theme mode**: `ThemeModeNotifier` infrastructure exists in `theme_color_notifier.dart` but the UI toggle was removed — app is always dark; `app.dart` uses only `darkWithSeed(seed)`; `lightWithSeed()` is a stub (returns dark theme); `profiles.theme_mode` column exists but is not read at runtime; SQL: `ALTER TABLE profiles ADD COLUMN IF NOT EXISTS theme_mode text DEFAULT 'dark'`
